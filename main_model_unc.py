...
from test_model_unc import test
from tcpnet_model_unc import TCPNet
...
def train_model(
    ...
    for epoch in range(1, epochs + 1):
        print(f"\nTraining for Epoch {epoch} and Sample {samples}\n")
        model.train()
        epoch_loss = 0
        train_loss = []
        loss_val = []
        ...
        with tqdm(total=len(data_tr), desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in data_tr:
                ...
                with torch.autocast(device.type if device.type != 'cuda' else 'cpu', enabled=amp):
                    ...
                    val_loss, iou_score = evaluate(model, data_val, device, amp, samples)
        ...
        if iou_score > best_iou:
            best_iou = iou_score
            torch.save(model, 'tcpnet_model_unc_samp_' + str(samples) + '.pt')
        print("Train_loss:", train_mean )
        print("Validation_loss", loss_val_mean )
        print("Validation IOU metric", iou_val_mean)

    print(f"\nModel has trained for sample {samples}\n")
...
if __name__ == '__main__':
    ...
    model = torch.load('tcpnet_model_unc_samp_' + str(args.sample) + '.pt')

    precision, recall, specificity, dice, iou, model_unc = test(model,dataset_test,device,args.sample)
