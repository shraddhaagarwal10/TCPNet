...
def test(model, dataloader, device, samples):
        ...
        # initialize metrcies to save model and data uncertainty values 
        model_unc= torch.empty(len(dataloader),2,128,128).to(device=device)
        data_unc= torch.empty(len(dataloader),2,128,128).to(device=device)
            ...
            for batch in tqdm(dataloader, total=len(dataloader)):
                ...
                with torch.no_grad():
                    for s in range(samples):
                        mask_pred, unc_pred = model(image)
        ...
        return prec_score, rec_score,spec_score,dice_score, iou_score, model_unc, data_unc