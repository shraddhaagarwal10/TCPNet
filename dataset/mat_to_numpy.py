...
data_dir = "/DATA/shraddha/brain-tumor-segmentation-unet/brain_tumor_dataset/"
...
for i, file in enumerate(files, start=1):
    ...
    mat_file = hdf5storage.loadmat(os.path.join(data_dir + 'data/', file))['cjdata'][0]
...
np.save(os.path.join(data_dir, 'pids.npy'), pids)
np.save(os.path.join(data_dir, 'images.npy'), images)
np.save(os.path.join(data_dir, 'masks.npy'), masks)

print('labels.npy, pids.npy, images.npy, masks.npy saved in', args.path)
